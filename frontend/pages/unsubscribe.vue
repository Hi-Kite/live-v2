<template>
  <div class="mx-auto flex w-full max-w-md flex-col gap-6 py-10">
    <div class="card space-y-4 p-8 text-center">
      <template v-if="state === 'loading'">
        <div
          ...
          aria-hidden="true"
        />
        <h1 class="text-xl font-bold">正在退订…</h1>
        <p class="text-sm text-soft">请稍候，正在为您处理退订请求。</p>
      </template>
      ...
      <template v-else-if="state === 'ok'">
        ...
        <p class="text-sm text-soft">您将不再收到开播通知。</p>
      ...
      <template v-else>
        ...
        <h1 class="text-xl font-bold">退订失败</h1>
        <p class="text-sm text-soft">{{ error }}</p>
        <div class="flex justify-center pt-1">
          <UiButton variant="secondary" to="/">返回首页</UiButton>
        </div>
      </template>
    </div>
  </div>
...
onMounted(async () => {
  try {
    ...
    state.value = 'error';
    error.value = apiErrorMessage(e, '链接无效或已过期');
  }
});
